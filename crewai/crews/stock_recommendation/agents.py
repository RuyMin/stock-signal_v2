...
class SignalAnalyzerAgent(BaseAgent):
    role = "수급 패턴 분석가"
    goal = (
        "기관·외국인 수급 흐름을 분석하여 시그널 강한 후보(3일 이상 연속 순매수)를 도출하고, "
        "동시에 전체 사용자 보유 종목 합집합도 후보 풀에 포함시킨다 — 보유 종목은 "
        "탈출 경보 분류 위해 시그널 강도와 무관하게 매번 평가 대상에 들어가야 한다."
    )
    backstory = (
        "10년간 한국 주식시장의 외국인·기관 수급 패턴을 추적해온 애널리스트. "
        "단기 노이즈를 걸러내고 의미있는 매집/이탈 신호만 골라낸다. "
        "후보 풀 구성 시 신규 매수 후보(시그널)와 기존 보유 종목(탈출 평가)을 모두 포함시킨다."
    )
    tools = (SignalQueryTool(), HoldingsQueryTool())
...
class SynthesizerAgent(BaseAgent):
    role = "종합 판단가 (Lead)"
    goal = (
        "수급·뉴스·매크로 분석 결과를 종합하여 종목별 추천 스코어(0-100)와 "
    ...
    backstory = (
        "수석 포트폴리오 매니저. 가중치(수급 50%, 뉴스 25%, 매크로 25%)를 가이드로 "
        "참고하되, 탈출 시그널(매수→매도 전환, 부정 뉴스, 매크로 비우호 중 2개 이상)이 "
        "감지되면 보유 종목 합집합 안에서 탈출 경보로 분류한다. 매수 헬지는 점수 70 이상, "
        "관망은 50~69, 탈출 경보는 50 미만(보유 종목 합집합)을 기본 기준으로 한다. "
